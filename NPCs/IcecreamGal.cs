...
namespace TheConfectionRebirth.NPCs
{
	...
	{
		#region Hostility
		private const float searchDistance = 26f; // Measured in tiles, how far to look for nearby players before attacking
		#endregion

		#region Movement
		private const float maxMoveSpeed = 3.5f; // Maximum number of pixels per update, while chasing the player
		private const float jumpProximity = 18f; // Measured in tiles.  Start jumping really high when within this horizontal distance from player target
		private const float acceleration = 0.4f; // Measured in pixels per update per update
		private const float jumpForce = 13f; // How hard to jump when jumping really high
		private const float hopForce = 4f; // How hard to jump when only hopping
		#endregion

		#region Star summoning
		private const int numberOfStars = 6; // How many stars can the NPC summon
		private const int averageNumberOfJumpsPerSummon = 4; // One in "X" chance that the NPC summons stars after a high jump
		private const float targetRadius = 90f; // The distance in pixels between the center of the NPC and the stars flying around it (i.e. the size of the star circle)
		private const float rotationSpeed = 0.045f; // How fast the stars fly around the NPC
		private const float summonSpeed = 4f; // How fast each star is summoned
		#endregion

		#region Attack behavior
		private const int starDamage = 30; // Damage that each star can cause
		private const int starKnockBack = 6; // Knock back from each star
		private const int shootingInterval = 25; // How many frames between the shooting of each star
