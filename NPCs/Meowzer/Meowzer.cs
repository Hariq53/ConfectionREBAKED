...
namespace TheConfectionRebirth.NPCs.Meowzer
{
	...
	{
		private NPC hitbox;
		private bool[] cannon = new bool[3];
		private float[] lockOn = new float[5];
		private Vector2 oldVel;
		private int lockDir;
		private bool[] sight = new bool[2];
		private Vector2 lastSeen;
		private float[] jitter = new float[3];
		private float[] recoil = new float[4];
		private int[] frame = new int[2];
		private int[] index = new int[2];
		private Vector2[] tailSeg = new Vector2[4];
		...
		public override void AI()
		{
			...
			void Fire()
			{
				...
				Projectile.NewProjectile(NPC.GetSource_FromAI("Fire"), firePos, val2 *= 0.5f, ModContent.ProjectileType<MeowzerBeam>(), Utilities.DamageLiteral(125), 2.5f);
				lockOn[3] = 1;
				lockOn[0] = 0;
				recoil[1] = 1;
	...
	public class MeowzerBeam : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Vector2 drawOrigin = new(tex.Width / 4, tex.Height / 2);
			drawOrigin.X -= 15f;
			drawOrigin.Y -= 25f;
			Rectangle refRect = new(tex.Width / 2, 0, tex.Width / 2, tex.Height);
			Rectangle rectangle = new(refRect.X, refRect.Y, tex.Width / 2, tex.Height / Main.projFrames[Projectile.type]);
