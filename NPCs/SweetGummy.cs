	...
	public class SweetGummy : ModNPC
	{
		...
		public override void Load()
		{
			...
			for (int i = 0; i < a.Length; i++)
			{
				if (a[i] == "Green")
				{
					VariationsManager<SweetGummy>.AddVariation(new(a[i], ModContent.Request<Texture2D>(Texture)));
					continue;
				}

				VariationsManager<SweetGummy>.AddVariation(new(a[i], ModContent.Request<Texture2D>(Texture + '_' + a[i]), b[i]));
			}
		}

		public override void Unload() => VariationsManager<SweetGummy>.ClearVariations();
		...
		public override bool PreAI()
		{
			if (Index == -1)
			{
				VariationsManager<SweetGummy>.GetRandomVariation(out Index);
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			DS.DrawNPC(NPC, VariationsManager<SweetGummy>.GetVariationByIndex(Index)!.Texture.Value, spriteBatch, screenPos, drawColor);
			return false;
		}
