...
namespace TheConfectionRebirth.Items.Weapons.Minions.RollerCookie
{
	public class SweetStaff : MinionWeaponBaseClass<RollerCookieSummonBuff, RollerCookieSummonProj>
	{
		...
		public override int Value => Item.buyPrice(0, 4, 0, 0);
	...
	{
		private const float blocksPerRotation = 9;
		private const float rotationPerBlock = 1 / blocksPerRotation;
		...
		private const int wallLatchTime = 30;
		private const int wallLatchTimePlusOne = wallLatchTime + 1;
		...
		private const int RIGHTWALL = 2;
		private const int CEIL = 3;

		private void SwitchAdhering()
		{
		...
		private void CreateDust() { CreateDust(Projectile); }
		...
		private void Unadhere() { Unadhere(Projectile, WallClimbState); }

		private void Adhere() { Adhere(Projectile, WallClimbState); }
		...
		private Vector2 RollerCookie_GetIdlePos_Floating(Player player)
		{
		...
		private Vector2 RollerCookie_GetIdlePos_Standing(Player player)
		{
