...
{
	public class GastropodSummonProj : ModProjectile
	{
		...
		public override void SetDefaults()
		{
			...
			shoot = ModContent.ProjectileType<GastropodSummonPinkLazer>();
			shootSpeed = 12f;
		}
		...
		public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<GastropodSummonBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<GastropodSummonBuff>()))
			{
				...
				if (target)
				{
					...
					if (Projectile.ai[1] == 0f)
					{
						...
						if (Main.myPlayer == Projectile.owner)
						{
							...
							shootVel *= shootSpeed;
							Projectile.frame = 3;
							int proj = Projectile.NewProjectile(new EntitySource_Misc(""), Projectile.Center.X, Projectile.Center.Y, shootVel.X, shootVel.Y, shoot, Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);
